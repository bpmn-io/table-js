...
function RowDrag(eventBus, sheet, elementRegistry) {
...
RowDrag.$inject = [ 'eventBus', 'sheet', 'elementRegistry' ];
...
RowDrag.prototype.updateVisuals = function(event) {
  ...
  this.dragVisual.style.left = this.previousCoordinates.x + 'px';
  this.dragVisual.style.top = this.previousCoordinates.y + 'px';

  // clear the indicator for the previous run
  if(this.highlightedBorder) {
    this.highlightedBorder.style.borderTop = '';
    this.highlightedBorder.style.borderBottom = '';
    this.highlightedBorder = null;
  }
  ...
  if(tr) {
    ...
    if(this._sheet.getBody().contains(tr)) {
      ...
      if(event.clientY < offset.y + tr.clientHeight / 2) {
        tr.style.borderTop = '4px dashed green';
        this.moveAbove = true;
      } else {
        tr.style.borderBottom = '4px dashed green';
        this.moveAbove = false;
      }
...
RowDrag.prototype.createDragVisual = function(element) {
  ...
  var table = domify('<table style="width: 100%; pointer-events:none; box-shadow: 5px 5px 3px 0px rgba(181, 181, 181, 0.7); transform: scale(0.75); transform-origin: left top;"><tbody></tbody></table>');
...
RowDrag.prototype.stopDragging = function() {
  if(this.highlightedBorder) {
    ...
    this._sheet.moveRow(this.draggedElement, targetElement, this.moveAbove);
  }
  if(this.dragVisual) {
