...
Sheet.prototype._addElement = function(type, element, parent) {
  ...
  var gfx = graphicsFactory.create(type, element, parent);
...
Sheet.prototype.addRow = function(row) {
  row.previous = row.previous || null;
  row.next = row.next || null;

  var r = this._addElement('row', row, this._rootNode);
...
Sheet.prototype.addColumn = function(column) {
  column.previous = column.previous || null;
  column.next = column.next || null;
