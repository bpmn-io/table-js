export default class Sheet {
  constructor(config, elementFactory, elementRegistry, eventBus) {
  ...
  getRoot() {
    if (typeof this._root === 'undefined') {
      this._root = this._elementFactory.createRoot({
        __implicit: true
      });
    }
...
Sheet.$inject = [ 'config', 'elementFactory', 'elementRegistry', 'eventBus' ];
